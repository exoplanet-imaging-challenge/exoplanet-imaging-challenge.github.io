...
The datasets used in this competition are diverse in terms of their characteristics and the observation conditions: from H-band (1.6 mum) to L-band (3.8 mum), slow/high speed cameras, broadband/barrowband filters, total range of rotation, with or without a coronagraph, etc. 
...
#### Injections
Using the center defined at we injected from 0 to 5 synthetic planetary signals in the data, with contrast and separation randomly picked around the detection limit (5-sigma contrast curve) computed with the baseline (a full frame PCA). For the injections, we made use of the VIP pipeline.
...
For the second sub-challenge on ADI+mSDI post-processing, each dataset will be composed of:
